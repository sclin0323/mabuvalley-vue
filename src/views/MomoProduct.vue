  ...
  setup() {
    ...
    const handleQuery = async () => {
      ...
      try {
        ...
        const url = $prefix + `api/momo/product/read?${params.toString()}`
    ...
    const openSyncProducts = () => {
      // 這裡設定您想要開啟的特定網址
      const url = 'https://mabuvalley-sync-py-production.up.railway.app/api/sync/momo/products'
    ...
    const renderColumn = (column, row) => {
      switch (column.prop) {
        ...
        case 'orderCounselQty':
          return h('span', {
            class: parseInt(row.orderCounselQty) <= 0 ? 'amount out-of-stock' : 'amount'
          }, `${row.orderCounselQty}`)
        case 'syslast':
          return h('span', {
            class: parseInt(row.syslast) <= 0 ? 'amount out-of-stock' : 'amount'
          }, `${row.syslast}`)
        default:
